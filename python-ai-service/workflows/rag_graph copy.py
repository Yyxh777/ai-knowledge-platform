...
# {}包围的是字典，[]包围的是列表
# 变量大写（指不要去修改，是常量），并且在模块顶层定义，是 模块级变量！
# 拓展：
# 1. 字典的获取方式有两种，一种是d["policy"]，一种是d.get("policy")，前者在key不存在时会报错，后者在key不存在时不会报错，会返回None
#   get方法可以设置默认值，如果key不存在，会返回默认值，如d.get("policy", "xx类")
# 2. 模块级变量在其他文件也可以获取，使用from config import XX
...
#  单下划线，表示私有变量，只能在当前模块中使用，也是模块级单例
#  双下划线，在单下划线的基础上加个名称修饰的功能，__a会被修饰为_ClassName__a，作用是避免子类变量覆盖父类变量
#  拓展：
#  1. 前后双下划线就是魔术方法，即python内部的方法，如__init__，__str__，__repr__，__del__等
#  2. 只要在模块顶层定义的变量都是单例！


# ────────────────────────────────────────────────────────────────────────────────

# 模块顶层定义的函数类似与java的public static方法，可以直接调用，不需要实例化
...
def web_search(query: str, limit: int = 5):
    ...
    return r_json
# requests第三方HTTP库，用于发送HTTP请求，比urllib更简单，比httpx更简单
# raise_for_status()方法用于检查HTTP请求是否成功，如果失败则抛出异常
...
def query_mysql(sql: str) -> str:
    ...
    if not sql_stripped.startswith("SELECT"):
        ...
        # dumps方法的作用是将字典转换为json字符串，ensure_ascii=False则是中文不转义，原文输出
    ...
    try:
        ...
        # 使用with语句，可以自动关闭连接，不需要手动关闭
        # cursor类似于执行器，执行sql语句，并返回结果
        with conn.cursor() as cursor:
            cursor.execute(sql)
            # fetchmany方法用于获取指定数量的行，限制返回最多100行，返回一个列表，列表中每个元素是一个字典
            rows = cursor.fetchmany(100)
            # []是列表，for row in rows是遍历rows为row给前面用，通过dict方法将row转换为字典
...
def _trim_messages(existing: list, new: list) -> list:
    ...
    # 列表拼接，取最后_MAX_STORED条数据，即从后往前数_MAX_STORED条数据
...
# typeddict是定义dict每个字段类型的数据结构，其本质还是dict，和普通类没有关系
# 相比与普通dict，会自动进行静态类型约束，如果类型不匹配，也不会影响运行
